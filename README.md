...
- `@first-project/ui`: a stub React component library shared by both `web` and
  `docs` applications
- `@first-project/eslint-config`: `eslint` configurations (includes
  `eslint-config-next` and `eslint-config-prettier`)
- `@first-project/typescript-config`: `tsconfig.json`s used throughout the
  monorepo
